...
def create_gaia_dataset_loader(use_raw_dataset: bool, set_to_run: str):
    ...
    # NEW: Carica direttamente il file JSONL invece di usare lo script deprecato
    data_file_path = f"data/gaia/2023/{set_to_run}/metadata.jsonl"
    
    if not os.path.exists(data_file_path):
        raise FileNotFoundError(f"GAIA data file not found at {data_file_path}. Please check the download.")

    print(f"Loading GAIA dataset from {data_file_path}...")
    
    # Carica direttamente il file JSONL
    eval_ds = datasets.load_dataset("json", data_files=data_file_path, split="train")

    # Rinomina le colonne per coerenza con il resto del codice
